"pluginSettings.vim 
"A file sourced by init.vim that sources plugin settings files.
source ~/.config/nvim/pluginSettings/airline.vim
"source ~/.config/nvim/pluginSettings/coc.vim
source ~/.config/nvim/pluginSettings/disaprove-deep-indentation.vim
source ~/.config/nvim/pluginSettings/fugitive.vim
source ~/.config/nvim/pluginSettings/fzf.vim
source ~/.config/nvim/pluginSettings/fzf-checkout.vim
source ~/.config/nvim/pluginSettings/nerdtree.vim
source ~/.config/nvim/pluginSettings/vim-go.vim

